...
def _make_partial_function_name(fn):
    """ Defines convention to labelling a partialised function. """
    assert isinstance(fn, partial), 'This function requires a partial fn.'
    out = fn.func.__name__
    for key, value in fn.keywords.iteritems():
        if callable(value):
